...
const guards = [
  ...
  ["sales-summary-bucket", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const salesSummaryBucketSchema = z.strictObject({", "const salesSummaryBucketSchema = z.looseObject({"],
  ["sales-summary-exclusions", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const salesSummaryExclusionsSchema = z.strictObject({", "const salesSummaryExclusionsSchema = z.looseObject({"],
  ["payment-paid-totals", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const paymentPaidTotalsSchema = z.strictObject({", "const paymentPaidTotalsSchema = z.looseObject({"],
  ["payment-refunded-totals", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const paymentRefundedTotalsSchema = z.strictObject({", "const paymentRefundedTotalsSchema = z.looseObject({"],
  ["payment-voided-totals", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const paymentVoidedTotalsSchema = z.strictObject({", "const paymentVoidedTotalsSchema = z.looseObject({"],
  ["payment-type-total", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const paymentTypeTotalSchema = z.strictObject({", "const paymentTypeTotalSchema = z.looseObject({"],
  ["payment-status-count", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const paymentStatusCountSchema = z.strictObject({", "const paymentStatusCountSchema = z.looseObject({"],
  ["item-sales-group", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const itemSalesGroupSchema = z.strictObject({", "const itemSalesGroupSchema = z.looseObject({"],
  ["cash-entry-type-total", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const cashEntryTypeTotalSchema = z.strictObject({", "const cashEntryTypeTotalSchema = z.looseObject({"],
  ["cash-drawer-reference", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const cashDrawerReferenceSchema = z.strictObject({", "const cashDrawerReferenceSchema = z.looseObject({"],
  ["cash-reason-reference", "nested Standard output schemas match invented complete results without closing Toast strings", reportToolsPath, "const cashReasonReferenceSchema = z.strictObject({", "const cashReasonReferenceSchema = z.looseObject({"],
