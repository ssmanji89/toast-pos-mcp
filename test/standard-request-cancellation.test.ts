...
test("abort during hierarchy sleep sends no upstream request", async () => {
  ...
  coordinator.record(
    { restaurantGuid: RESTAURANT_GUID, apiKey: "orders", endpointKey: "orders/v2/payments" },
    { primary: "GLOBAL", account: false, remaining: 0, resetAtEpochMs: START_EPOCH_MS + 1_000, retryAfterEpochMs: undefined },
  );
  const client = makeClient({
    now: () => nowMs,
    coordinator,
  ...
  releaseSleep();
  const later = await client.getJsonDetailedCancellable(scopedRequest("later", "/orders/v2/payments"));
...
test("abort during in-flight fetch aborts upstream and later traffic recovers", async () => {
  ...
  const controller = new AbortController();
  const inFlight = client.getJsonDetailedCancellable(scopedRequest("in-flight"), { signal: controller.signal });
...
test("abort during retry backoff prevents retry fetch", async () => {
  ...
  const client = makeClient({
    ...
    dataFetch: async () => {
      dataCalls += 1;
      if (dataCalls === 1) return new Response("{}", { status: 503, headers: { "content-type": "application/json" } });
      return jsonResponse({ recovered: true });
    },
  });
  const controller = new AbortController();
  const retrying = client.getJsonDetailedCancellable(scopedRequest("retry"), { signal: controller.signal });
  await sleepStartedPromise;
  controller.abort(ABORT_MARKER);
...
function scopedRequest(rateLimitKey: string, path: `/${string}` = "/restaurants/v1/restaurants/example") {
...
function tokenResponse(): Response {
  return jsonResponse({ token: { tokenType: "Bearer", expiresIn: 3600, accessToken: "synthetic-cancellation-token" } });
}
