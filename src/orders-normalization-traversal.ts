...
function normalizeAppliedTaxes(sources: readonly SourceAppliedTax[], seen: Set<string>): readonly NormalizedAppliedTax[] { return Object.freeze(sources.map((source) => { const guid = source.guid.toLowerCase(); assertUnique(seen, guid, "applied tax"); return Object.freeze({ guid, taxRate: normalizeReference(source.taxRate), rate: source.rate == null ? undefined : exactDecimalFromNumber(source.rate), taxAmount: exactDecimalFromNumber(source.taxAmount), type: source.type ?? undefined, facilitatorCollectAndRemitTax: source.facilitatorCollectAndRemitTax ?? undefined }); })); }
