...
function mergeMenuItemGroups(left: MenuItemDimension, right: MenuItemDimension): MenuItemDimension { const groups = new Map<string, MenuItemGroupDimension>(); for (const group of [...left.itemGroups, ...right.itemGroups]) groups.set(group.multiLocationId === undefined ? group.guid : `${group.guid}:${group.multiLocationId}`, group); return Object.freeze({ ...left, itemGroups: Object.freeze([...groups.values()].sort((a, b) => a.guid.localeCompare(b.guid) || (a.multiLocationId ?? "").localeCompare(b.multiLocationId ?? ""))) }); }
