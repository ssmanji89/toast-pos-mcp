...
Capability decisions are now represented as deterministic, read-only helper functions before any Toast data tool is registered. The capability model decodes advisory `scope`/`scp` claims from bearer-token payloads when they are JWT-shaped, preserves unknown scope strings that use the same safe scope syntax, and treats malformed or opaque tokens as advertising no scopes. JWT scopes are display metadata only: observed Toast authorization remains authoritative, and any upstream authorization denial for a restaurant can override an advertised scope. Missing required scopes, guest-linked excluded scopes, restaurant mismatches, or observed authorization contradictions produce a structured `denied` decision with required, available, missing, and denied scope lists instead of an empty successful report. Remembered capability views are stored only in memory and are keyed by both runtime-config identity and restaurant GUID, so one credential or location cannot inherit another location's capability state.

Analytics job creation and Toast data tools belong to later slices; this runtime still registers no Toast data tools.
...
T0-001 established the reviewed public-use foundation. T1-001 added the local TypeScript `stdio` runtime and synthetic fixture harness. T1-002 added non-persistent runtime configuration loading, Zod validation, and the explicit Merchant-AI-consent acknowledgment gate. T1-003 added the in-memory OAuth client-credentials token lifecycle. T1-004 added the shared read-only Toast HTTP transport with structured errors, in-memory rate-limit state, and bounded retries. T1-005/T1-006 added both Standard API pagination families. T2-002 adds scope decoding and deterministic capability-denial helpers without registering Toast data tools. Location discovery and reports remain intentionally absent.
